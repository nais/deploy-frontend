...
it('(Fetch apikeys saga) Fetches list of apikeys', () => {
  ...
  return expectSaga(fetchApiKeys)
...
it('(Fetch apikeys saga) Handles errors from backend', () => {
  ...
  return expectSaga(fetchApiKeys)
    .withReducer(apiKeysReducer)
    .provide([[matchers.call.fn(httpGet), throwError(new Error('Error message'))]])
    .dispatch(action)
    .hasFinalState({ data: [], status: 'ERROR', errorMessage: new Error('Error message') })
    .run()
})
...
it('(Rotate apikeys saga) Handles errors when rotating key', () => {
  ...
  return expectSaga(rotateKey, action)
    .withReducer(rotateKeyReducer)
    .provide([[matchers.call.fn(httpPost), throwError(new Error('Error message'))]])
    .hasFinalState({
      status: 'ERROR',
